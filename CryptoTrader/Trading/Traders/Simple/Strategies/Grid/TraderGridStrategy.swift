...
class TraderGridStrategy: SimpleTraderStrategy {
    ...
    func updateTicker(bid: Double, ask: Double) {
        ...
        for order in gridMarketPositions {
            if shouldBuy(order: order, askPrice: ask) {
                buy(position: order)
            } else if shouldSell(order: order, bidPrice: bid) {
                sell(position: order)
            }
        }
    }
    ...
    func summary(shouldPrint: Bool) -> String {
        ...
        for order in self.gridMarketPositions.filter({$0.active}) {
            ...
            summaryString += "Order: \(order.qty £ 5)  bought @ \(order.price  £ 2) = \(order.value £ 2) | Current value: \(order.qty * currentBidPrice £ 2) (\(profit £ 2) / \(profitPercent.percentage £ 2)%)\n"
        }
        ...
        summaryString += "Net worth: \(currentBidPrice + gridMarketPositions.reduce(0, {result, order in result + order.qty * currentBidPrice}) £ 3)\n"
    ...
    private func updateGrid() {
        ...
        if currentPrice > maxGridPosition {
            gridMarketPositions.append(GridMarketPosition(targetPriceBottom: maxGridPosition +% config.gridSizePercent))
        }
        if currentPrice < minGridPosition {
            gridMarketPositions.append(GridMarketPosition(targetPriceBottom: minGridPosition -% config.gridSizePercent))
        }
    }
    ...
    private func shouldSell(order: GridMarketPosition, bidPrice: Double)  -> Bool {
        ...
        if order.profitStopLoss == 0 {
            // Update maybe
            if Percent(differenceOf: bidPrice, from: order.price) > config.profitMin {
                order.profitStopLoss = bidPrice -% config.profitStopLoss
            }
        } else {
            if bidPrice -% config.profitStopLoss > order.profitStopLoss {
                order.profitStopLoss = bidPrice -% config.profitStopLoss
            }
            else if (bidPrice < order.profitStopLoss) {
                return true
            }
        }
