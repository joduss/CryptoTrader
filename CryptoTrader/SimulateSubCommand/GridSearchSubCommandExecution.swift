...
class GridSearchSubCommandExecution {
    ...
    func executeBTS(config: TraderBTSStrategyConfig, dateFactory: DateFactory) -> (Double, String){
        ...
        return simulation.simulate(config: config, printPrice: false)
    }
    ...
    private func gridSearchMacd() {
        ...
        // BTC.
        for maxOrdersCount in [12, 15, 18] {
            for macdPeriod in [1, 5, 10] {
                for minDistancePercentBelow in [-0.6, -0.8, -1, -1.2, -1.4, -1.8] {
                    for minDistancePercentAbove in [0.5, 0.8, 1.3, 1.8] {
                        for minProfitsPercent in [0.35, 0.6, 0.9, 1.2, 1.6, 2] {
